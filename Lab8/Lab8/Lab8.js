...
let GET_handler = (req, res) => {
    ...
    switch ('/' + GetUrlPart(Path_forGet, 1)) {
        ...
        case '/connection': //текущее значение системного параметра KeepAliveTimeout.
        ...
        case '/close': //сообщение о закрытии сервера и остановите сервер через 10 секунд
            res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
        ...
        case '/headers': //все заголовки запроса и ответа
        ...
        case '/socket': //выведите ip- адрес, порт клиента и ip-адрес и порт сервера
            let sockinfo = '';
        ...
        case '/upload': //высылается web-форма позволяющая отправить POST-запрос к /upload, пересылающий серверу файл.
            console.log('Get Upload');
            res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
            res.end(fs.readFileSync(__dirname + "/Update.html"));
            break;

        case '/files': //высылается ответ, пересылающий  файл с именем filename
