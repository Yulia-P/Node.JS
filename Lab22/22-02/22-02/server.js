    ...
    if (req.cookies.refreshToken) {
        jwt.verify(req.cookies.refreshToken, refreshKey, async (err, user) => {
            if (err) console.log(err.message);
                ...
                redisClient.set(oldRefreshKeyCount, req.cookies.refreshToken, () => console.log('set refresh token'));
                redisClient.get(oldRefreshKeyCount, (err, result) => console.log('added refresh token:', result));
                oldRefreshKeyCount++;
                ...
                res.status(200).send(req.cookies.refreshToken);
            }
        });
    }
    else res.status(401).send('not');
});
